  ...
  if (loading || !authChecked) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        <div className="glass p-8 rounded-3xl animate-pulse text-foreground/40 text-xs font-bold tracking-widest uppercase">
  ...
  return (
    <div className="min-h-screen pt-28 pb-24 px-6 bg-background">
      <div className="max-w-7xl mx-auto animate-fade-in text-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 px-2">
          <div className="space-y-1">
            <button
              ...
              className="group flex items-center gap-2 text-foreground/40 hover:text-foreground transition-colors text-xs font-bold uppercase tracking-widest mb-4"
            >
            ...
            <div className="flex items-center gap-5">
              ...
              <h1 className="text-4xl font-bold tracking-tight text-white italic">
                {project.name} <span className="text-white/20 font-medium not-italic ml-2">Editor</span>
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              disabled={launching}
              className="px-6 py-3 rounded-[1.25rem] bg-accent-sky/10 hover:bg-accent-sky/20 border border-accent-sky/10 text-accent-sky text-[10px] font-bold uppercase tracking-widest transition-all disabled:opacity-50"
            >
              {launching ? "Sending..." : "Notify Subscribers"}
            </button>
            <div className="h-10 w-px bg-white/5 mx-1 hidden md:block" />
            <label className="flex items-center gap-3 glass px-5 py-3 rounded-2xl cursor-pointer hover:bg-white/5 transition-all">
              <span className="text-[10px] font-bold uppercase tracking-widest text-white/30">Visibilty:</span>
              <input
                type="checkbox"
        ...
        {launchMessage && (
          <div className="mb-8 p-4 rounded-2xl bg-accent-sky/10 border border-accent-sky/20 text-accent-sky text-xs font-medium animate-fade-in flex items-center gap-3">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="glass p-8 rounded-[3rem] space-y-8 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                ...
                <span className="text-[10px] uppercase tracking-widest text-white/20 font-bold">General</span>
              </div>

              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Project Name</label>
                  <input
                    value={project.name || ""}
                    onChange={(e) => setField("name", e.target.value)}
                    className="w-full rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-white focus:border-accent-sky/30 focus:bg-white/10 outline-none transition-all shadow-inner"
                    placeholder="E.g. My Awesome SaaS"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Unique Slug (URL)</label>
                  <input
                    readOnly
                    value={project.slug || ""}
                    className="w-full rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-white/30 font-mono shadow-inner cursor-not-allowed opacity-60"
                  />
                </div>
              </div>

              <div className="space-y-3">
                <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Tagline (Short Brief)</label>
                <textarea
                  rows={2}
                  value={project.short_description || ""}
                  onChange={(e) => setField("short_description", e.target.value)}
                  className="w-full rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-white focus:border-accent-sky/30 focus:bg-white/10 outline-none transition-all resize-none font-normal shadow-inner leading-relaxed"
                  placeholder="One sentence about your project..."
                />
              </div>

              <div className="space-y-3">
                <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Full Project Description</label>
                <textarea
                  rows={6}
                  value={project.full_description || ""}
                  onChange={(e) => setField("full_description", e.target.value)}
                  className="w-full rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-white focus:border-accent-sky/30 focus:bg-white/10 outline-none transition-all resize-none font-normal leading-relaxed shadow-inner"
              ...
              <div className="grid md:grid-cols-2 gap-8 pt-4">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">External Link</label>
                  <input
                    value={project.external_url || ""}
                    onChange={(e) => setField("external_url", e.target.value)}
                    className="w-full rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-white/60 focus:border-accent-sky/30 focus:bg-white/10 outline-none transition-all text-xs shadow-inner"
                    placeholder="https://yourapp.com"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">GitHub Repository</label>
                  <input
            ...
            <section className="glass p-8 rounded-[3rem] space-y-6">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                ...
                <span className="text-[10px] uppercase tracking-widest text-white/20 font-bold">Showcase List</span>
              </div>
              ...
              <div className="space-y-4">
                {(project.features || []).map((f: string, i: number) => (
                  <div key={i} className="group relative flex items-center gap-4 animate-fade-in translate-y-0 hover:-translate-y-1 transition-all">
                    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center text-[10px] font-bold text-white/20 group-hover:text-accent-sky group-hover:bg-accent-sky/5 transition-all">
                      {i + 1}
                    </div>
                    <input
                      value={f}
                      ...
                      className="flex-1 rounded-2xl border border-white/5 bg-white/5 px-6 py-4 text-sm text-white focus:border-accent-sky/30 focus:bg-white/10 outline-none transition-all shadow-inner"
              ...
              <button
                className="w-full py-5 rounded-[1.5rem] border border-dashed border-white/10 text-white/30 hover:text-white hover:bg-white/5 hover:border-white/20 transition-all text-[11px] font-bold uppercase tracking-[0.2em]"
            ...
            <section className="glass p-8 rounded-[3rem] space-y-6">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <h2 className="text-xl font-black text-white italic">Protocol Intel (FAQ)</h2>
                <span className="text-[10px] uppercase tracking-widest text-white/20 font-bold">Query Matrix</span>
              </div>

              <div className="space-y-6">
                {(project.faqs || []).map((faq: FAQ, i: number) => (
                  <div key={i} className="group glass p-6 rounded-[2rem] bg-white/[0.02] border-white/5 flex flex-col gap-4 relative animate-fade-in order-last">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-widest text-accent-sky/40">Query Node {i + 1}</span>
                      <button
                        onClick={() => {
                    ...
                    <textarea
                      ...
                      className="w-full bg-transparent border-none p-0 text-xs text-white/40 placeholder:text-white/10 outline-none resize-none font-light italic leading-relaxed"
              ...
              <button
                className="w-full py-5 rounded-[1.5rem] border border-dashed border-white/10 text-white/20 hover:text-white hover:bg-white/5 hover:border-white/20 transition-all text-[11px] font-black uppercase tracking-[0.3em] bg-white/[0.01]"
          ...
          <div className="space-y-8">
            ...
            <section className="glass p-8 rounded-[2.5rem] space-y-8">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Hero Section Content</h2>
              </div>

              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Hero Headline</label>
                  <input
                    placeholder="The main title on your landing page"
                    value={project.hero_title || ""}
                    onChange={(e) => setField("hero_title", e.target.value)}
                    className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-xs text-white outline-none focus:border-accent-sky/30 shadow-inner"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Hero Description</label>
                  <textarea
                    rows={4}
                    placeholder="Short mission statement"
                    value={project.hero_subtitle || ""}
                    onChange={(e) => setField("hero_subtitle", e.target.value)}
                    className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-xs text-white outline-none focus:border-accent-sky/30 resize-none font-normal leading-relaxed shadow-inner"
                  />
                </div>
              </div>
            </section>
            ...
            <section className="glass p-8 rounded-[2.5rem] space-y-8">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Project Gallery</h2>
              </div>

              {/* URL INJECTION (FREE WORKAROUND) */}
              <div className="space-y-5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-white/20 ml-2">Import via Image URL (Free Hosting)</label>
                <div className="flex flex-col gap-3">
                  <input
                    placeholder="https://images.unsplash.com/..."
                    value={newImageUrl || ""}
                    onChange={(e) => setNewImageUrl(e.target.value)}
                    className="w-full rounded-xl border border-white/5 bg-white/5 px-5 py-4 text-[10px] text-white outline-none focus:border-accent-sky/30 shadow-inner"
                  />
                  <button
                    onClick={() => {
              ...
              <div className="flex items-center gap-4 py-4 opacity-20">
                ...
                <span className="text-[8px] font-bold text-white/50 uppercase tracking-[0.4em]">OR</span>
                <div className="h-px flex-1 bg-white/5" />
              </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                <label className="flex flex-col items-center justify-center py-12 rounded-[2.5rem] border-2 border-dashed border-accent-emerald/20 bg-accent-emerald/[0.02] hover:bg-accent-emerald/5 hover:border-accent-emerald/40 cursor-pointer transition-all group relative overflow-hidden">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent-emerald/60 text-center px-8 leading-relaxed">Free Cloud Upload <br /><span className="text-[8px] font-normal opacity-50">via ImgBB Integration</span></span>
                  <input
                    type="file"
                    accept="image/*"
                ...
                <div className="flex items-center gap-4 py-2 opacity-10">
                  <div className="h-px flex-1 bg-white/5" />
                  <span className="text-[8px] font-bold text-white uppercase tracking-[0.3em]">SECURE ADMID STORAGE</span>
                  <div className="h-px flex-1 bg-white/5" />
                </div>

                <label className="w-full flex flex-col items-center justify-center py-8 rounded-[2.5rem] border-2 border-dashed border-white/5 hover:bg-white/5 hover:border-white/10 cursor-pointer transition-all group relative overflow-hidden text-white/5">
                  {uploading && (
                    <div className="absolute inset-0 bg-background/80 flex items-center justify-center z-10 animate-fade-in">
              ...
              <div className="grid grid-cols-1 gap-6 mt-8">
                {(project.gallery || []).map((url: string, i: number) => (
                  <div key={i} className="group relative aspect-video rounded-[2rem] overflow-hidden glass border-white/5 shadow-2xl">
                    <img
                      src={url}
        ...
        <div className="mt-20 pt-16 border-t border-white/5 flex flex-col items-center gap-8">
          <div className="flex flex-col items-center gap-4 mb-4">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-white/10 italic">Ready to publish</p>
          </div>

          <button
            onClick={updateProject}
            disabled={saving}
            className="w-full max-w-2xl py-6 rounded-[2.5rem] bg-white text-black font-bold uppercase tracking-[0.3em] text-xs hover:bg-white/90 hover:scale-[1.01] active:scale-[0.99] transition-all shadow-2xl shadow-sky-500/10 flex items-center justify-center gap-4 disabled:opacity-50 border-b-4 border-black/10"
          >
